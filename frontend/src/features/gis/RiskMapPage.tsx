...
const legendItems: { level: string; color: string }[] = [
...
export function RiskMapPage() {
  ...
  useEffect(() => {
    Promise.all([api.getRiskMap(), api.getAlerts()]).then(([v, a]) => {
      setVillages(v);
      setAlerts(a);
      setLoading(false);
    });
  ...
  const villageAlerts = selected ? alerts.filter((a) => a.villageId === selected.id) : [];

  if (loading) return <LoadingState message="Loading map data..." />;

  return (
    <div>
      <PageHeader title="Village Risk Map" subtitle="Geographic view of disease risk across all monitored villages." breadcrumb={['Home', 'Risk Map']} />
                ...
                >
                  <Popup>
                    <div className="space-y-1">
                      ...
                      <Link to={`/villages/${v.id}`} className="text-xs font-medium text-primary-600 hover:underline">View Village Details</Link>
                    </div>
                  </Popup>
                </CircleMarker>
              ))}
          ...
          {!selected ? (
            ...
            <div className="flex h-full flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-5">
                ...
                <div className="mb-5">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="rounded-lg bg-slate-50 p-3">
                      ...
                      <p className="text-lg font-bold" style={{ color: riskConfig[selected.riskLevel].hex }}>{selected.riskScore}%</p>
                    </div>
                    <div className="rounded-lg bg-slate-50 p-3">
                      <p className="text-xs text-slate-500">Suspected Cases</p>
                      <p className="text-lg font-bold text-slate-900">{selected.suspectedCases}</p>
                    </div>
                    <div className="rounded-lg bg-slate-50 p-3">
                ...
                <Link to={`/villages/${selected.id}`} className="btn-primary w-full">View Full Village Profile</Link>
              </div>
            </div>
          )}
        </div>
