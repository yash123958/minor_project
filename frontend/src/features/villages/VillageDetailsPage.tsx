...
import { alerts as mockAlerts, healthRecords } from '@/data/mockData';
...
export function VillageDetailsPage() {
  ...
  useEffect(() => {
    Promise.all([api.getVillage(id!), api.getAlerts(), api.getTrends(id!, 30)]).then(([v, a, t]) => {
      if (v) {
        setVillage(v);
  ...
  if (error || !village) return <ErrorState message="Village not found." onRetry={() => navigate('/villages')} />;

  const villageRecords = healthRecords.filter((r) => r.villageId === id);
  ...
  return (
    <div>
      <PageHeader
        ...
        breadcrumb={['Home', 'Villages', village.name]}
        actions={
          <div className="flex items-center gap-2">
            <RiskBadge level={village.riskLevel} />
            <button onClick={() => navigate('/villages')} className="btn-secondary"><ArrowLeft className="h-4 w-4" /> Back</button>
          </div>
        }
      />
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <ChartCard title="Health" subtitle="Disease distribution">
      ...
      {/* Health Records */}
      <div className="card mt-6 p-5">
        <h3 className="mb-4 text-sm font-semibold text-slate-900">Health Records</h3>
        {villageRecords.length === 0 ? (
          ...
          <div className="space-y-2">
            {villageRecords.slice(0, 5).map((r) => (
              <Link key={r.id} to={`/health/${r.id}`} className="flex items-center justify-between rounded-lg border border-slate-100 p-3 hover:bg-slate-50">
                <div>
